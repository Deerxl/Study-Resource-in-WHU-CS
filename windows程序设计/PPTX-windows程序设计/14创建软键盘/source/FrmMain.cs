...
namespace softKB
{
    public partial class FrmMain : NoActivateWindow
    {
...
this.Activate();
        ...
        void KeyButton_Click(object sender, EventArgs e)
        {
            ...
            if (btn.IsLockKey)
            {
                KeyboardInput.SendToggledKey(btn.KeyCode);
            }
        ...
        void InitializeKeyButtons()
        {
            ...
            short numLockState = UnsafeNativeMethods.GetKeyState((int)Keys.NumLock);
            keyButtonNumLock.IsPressed = (numLockState & 0x0001) != 0;


            short scrLockState = UnsafeNativeMethods.GetKeyState((int)Keys.Scroll);
            keyButtonScrollLock.IsPressed = (scrLockState & 0x0001) != 0;

            var keysMappingDoc = XDocument.Load("KeysMapping.xml");
